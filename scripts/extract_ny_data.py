...
def download_cms_data(url: str, output_path: str) -> bool:
    ...
    try:
        ...
        if response.headers.get('content-type', '').startswith('text/html'):
            ...
            if not download_url:
                print("Could not find direct CSV download link. Trying alternative approach...")
...
def extract_ny_data(input_file: str, output_file: str) -> bool:
    ...
    try:
        ...
        ny_df = ny_df.dropna(subset=['provider_id', 'provider_name', 'ms_drg_definition'])
        ...
        for col in numeric_columns:
            if col in ny_df.columns:
                ...
                ny_df[col] = ny_df[col].astype(str).str.replace('$', '').str.replace(',', '')
        ...
        if 'provider_zip_code' in ny_df.columns:
            ny_df['provider_zip_code'] = ny_df['provider_zip_code'].astype(str).str.strip()
